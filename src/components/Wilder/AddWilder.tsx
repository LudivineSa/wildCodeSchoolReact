...
function AddWilder() {
    ...
    const sendData = async (e) => {
        e.preventDefault();
        const result = await addWilder(inputName.current.value, inputCity.current.value, skills)
        ...
        <div>
            <form action="" className="center white">
                ...
                <div className="form">
                    ...
                    <input type="number"  defaultValue="0" min="0" max="10" ref={inputSkillVote} />    
                </div>  

                <Button variant="contained" onClick={addSkill} sx={{
                ...
                <div className="flex">
                    {skills && skills.map((skills, i) => (
                        <div className="skills" key={i*(Math.random()*100)}>
                            <p className="closeSkill" onClick={() => deleteSkill(i)}>X</p>
                            <p className="vote">{skills.votes}</p>    
                            <p className="title">{skills.title}</p>
                        </div>
                    ))}
                </div>
                <Button variant="contained" onClick={sendData} sx={{
